...
def greedy_decode_logits_to_strings(logits, charset):
    preds = logits.argmax(2).transpose(0,1)
...
def main():
    img_folder = "images/train_real1/img1"
    labels_file = "images/train_real1/train_real1.txt"
    ...
    loader = DataLoader(dataset, batch_size=16, shuffle=True, collate_fn=collate_fn, num_workers=4, pin_memory=True)

    model = CRNN(img_h=32, num_channels=1, num_classes=len(charset)).to(device)
    phase1_ckpt = "checkpoints/phase2_best.pth"
    if os.path.exists(phase1_ckpt):
        model.load_state_dict(torch.load(phase1_ckpt, map_location=device))
        print(f"Loaded {phase1_ckpt}")
    ...
    for epoch in range(1, 2):
        ...
        for imgs, labels, widths in tqdm(loader, desc=f"[phase2] Epoch {epoch}"):
            ...
            imgs_keep = imgs[keep_idx].to(device)
        ...
        print(f"[phase2] Epoch {epoch} avg loss: {avg_loss:.4f} avg CER (char-weighted): {avg_cer:.4f} skipped_samples_this_epoch: {skipped_samples_total}")
        ...
        if avg_cer < best_cer:
            ...
            print("Saved checkpoints/phase2_best.pth")
